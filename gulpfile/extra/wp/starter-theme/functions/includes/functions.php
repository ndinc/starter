...
function get_head_meta($name=null){
  ...
  if (is_home()) {
    $meta = array(
      'title' => strip_tags(isset($_META['title'])? $_META['title'] : get_bloginfo('name')),
      'url' => 'http://'.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'],
      'description' => strip_tags( isset($_META['description'])? $_META['description'] : get_bloginfo('description') ),
      'image' => $_META['image'],
    ...
    $meta = array(
      'title' => strip_tags( (isset($title)? $title.' | ' : ''  ).$_META['title'] ),
      'url' => 'http://'.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'],
      'description' => str_replace(array("\r\n","\n","\r"), '', strip_tags( ( isset($desc)? $desc : $_META['description']  )) ),
      'image' => isset($image)? $image : $_META['image'],
      'site_name' => $_META['site_name'],
      'type' => 'article'
    );
  }else if( (is_static() or is_page()) and isset($_PAGES[get_page_slug()]) ){
    $p = $_PAGES[get_page_slug()];
    $image = !empty($p['image'])? $p['image'] : $_META['image'];
    $desc = !empty($p['description'])? $p['description'] : $_META['description'];
    $meta = array(
      'title' => $p['title'].' | '.$_META['title'],
      'url' => 'http://'.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'],
      'description' => $desc,
      'image' => $image,
      'site_name' => $_META['site_name'],
    ...
    $image = get_post_main_image($post->ID);
    $meta = array(
      'title' => ( !empty($title)? $title.' | ' : '' ).$_META['title'],
      'url' => 'http://'.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'],
      'description' => ( !empty($desc)? $desc : $_META['description']  ),
      'image' => isset($image)? $image : $_META['image'],
      'site_name' => $_META['site_name'],
      'type' => 'article'
    );
    ...
    $meta = array(
      'title' => (!empty($term)? $term->name.' | ' : '' ).$_META['title'],
      'url' => 'http://'.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'],
      'description' => !empty($term)? $term->description : $_META['description'],
      'image' => !empty($image)? $image['url'] : $_META['image'],
      'site_name' => $_META['site_name'],
      'type' => 'article'
    );
  }else{
