...
function is_prod() {
  ...
  return strpos($_SERVER['HTTP_HOST'], $_DOMAIN['production']) !== false;
}

function is_staging() {
  global $_DATA;
  $_DOMAIN = $_DATA['domain'];
  return strpos($_SERVER['HTTP_HOST'], $_DOMAIN['staging']) !== false;
}
...
function is_develop() {
  ...
  }else if(strpos($_SERVER['HTTP_HOST'], $_DOMAIN['develop']) !== false){
    $is_develop = true;
  }
  return $is_develop;
}
